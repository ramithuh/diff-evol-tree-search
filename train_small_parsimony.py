...
if(args['log_wandb']):
    ...
    wandb.init(project=args['project'], name = args['notes'] + metadata['exp_name'], entity="<your_username>", config = metadata, tags=["small_parsimony", args['tags']], notes = args['notes'])
...
seq_optimizer = OptaxSolver(opt = optax.adam(metadata['lr_seq'], eps_root = 1e-16), fun = inner_objective) 
seq_tree_init = jax.vmap(seq_optimizer.init_state, (0, 0, None),0)
inner_opt_state = seq_tree_init(seq_params, tree_params, [seqs, metadata, metadata['tLs'][0], 0])
jitted_seq_update = jit(vmap(seq_optimizer.update, (0, 0, 0, None),0))

# inner_opt_state = seq_optimizer.init_state(seq_params, tree_params, [seqs, metadata, metadata['tLs'][0], 0])
...
for _ in range(metadata['epochs']):
    ...
    seq_params, inner_opt_state = jitted_seq_update(seq_params, inner_opt_state, tree_params, [seqs, metadata, metadata['tLs'][0],_])
    ...
    if(cost.min() < best_ans):
        if(_%20==0):
            print_success_info("Found a better seq at epoch %d with cost %f from seq %d. (delta at epoch = %d) \n" % (_, cost.min(), pos, cost.max()-cost.min()))
        best_ans = cost.min()
